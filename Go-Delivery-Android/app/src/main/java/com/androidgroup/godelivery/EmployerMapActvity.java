...
public class EmployerMapActvity extends FragmentActivity implements
    ...
    List<String> LatitudeStringsList = new ArrayList<String>();
    List<String> LongitudeStringsList = new ArrayList<String>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        jobStatusTitle = (TextView) findViewById(R.id.jobCreaterActivityStatusTitleID);
        jobStatus = (TextView) findViewById(R.id.jobCreaterActivityStatusID);

        jobDetailsButton = (Button) findViewById(R.id.jobCreaterActivityJobDetailsButtonID);
        cancelButton = (Button) findViewById(R.id.jobCreaterActivityCancelButtonID);
    ...
    private class FetchAcceptedJobDetails extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            // jobLongTitle.setText("Your Job for the Delivery of " + jobDetails[9] +" has been Accepted By");
            //  jobAccepterUsername.setText("Name\n" + jobDetails[3]);
            // jobAccepterContactNumber.setText("Contact Number\n" + jobDetails[6]);
    ...
    public void jobCreaterActivityJobDetailsButton(View v)
    {
    ...
    private class FetchLocationDetails extends AsyncTask<String, Void, String> {
        @Override
        protected String doInBackground(String... urls) {

            // params comes from the execute() call: params[0] is the url.
            try {
                return FetchLocation(urls[0]);
            } catch (IOException e) {
                return "Unable to retrieve web page. URL may be invalid.";
            }
        }
        ...
        protected void onPostExecute(String result) {
            ...
            if (result.equals("OK"))
            {
                ...
                // Toast.makeText(getApplicationContext() , "Location Tracking Updated!", Toast.LENGTH_SHORT).show();
    ...
    private String FetchLocation(String myurl) throws IOException, UnsupportedEncodingException {
        InputStream is = null;
        ...
        try {
            ...
            boolean FirstLineLatitude = true;
            boolean AddLatitudeLongitudeToList = false;

            LatitudeStringsList.clear();
            LongitudeStringsList.clear();
            ...
            while ((readlineTextListing = textReader.readLine()) != null) {
                ...
                if (readlineTextListing.equals("***"))
                {

                    continue;
                }

                if (FirstLineLatitude)
                {
                    FirstLineLatitude = false;

                    Latitude = readlineTextListing;

                }

                else
                {
                    FirstLineLatitude = true;

                    Longitude = readlineTextListing;

                    AddLatitudeLongitudeToList = true;
                }


                if (AddLatitudeLongitudeToList)
                {
                    AddLatitudeLongitudeToList = false;
                    ...
                    {
                        LatitudeStringsList.add(Latitude);
                        LongitudeStringsList.add(Longitude);
    ...
    private void setUpMapIfNeeded() {
        ...
        if (mMap == null) {
            ...
            SupportMapFragment mapFragment = ((SupportMapFragment) getSupportFragmentManager()
                    .findFragmentById(R.id.mapJobCreater));
    ...
    private void setUpMap() {
        ...
        if (LatitudeStringsList.size() > 0 ) {
            ...
            for (int i = 0; i < LatitudeStringsList.size(); ++i) {


                Latitude = Double.valueOf(LatitudeStringsList.get(i));
                Longitude = Double.valueOf(LongitudeStringsList.get(i));

                LatLng polyline = new LatLng(Latitude,
                        Longitude);
            ...
            mMap.addMarker(new MarkerOptions().position(new LatLng(Double.valueOf(LatitudeStringsList.get(LatitudeStringsList.size() - 1)), Double.valueOf(LongitudeStringsList.get(LatitudeStringsList.size() - 1)))).title("Driver").icon(BitmapDescriptorFactory.fromResource(R.drawable.bicycle_icon)));


            mMap.addPolyline(new PolylineOptions().addAll(polylines).width(16).color(Color.BLUE));


            LatLng CurrentPosition = new LatLng(Double.valueOf(LatitudeStringsList.get(LatitudeStringsList.size() - 1)), Double.valueOf(LongitudeStringsList.get(LatitudeStringsList.size() - 1)));
...
// bounds
    ...
    public void LogOutClicked(View v)
    {
        ...
        Intent intent = new Intent(EmployerMapActvity.this, AlreadyLoggedInActivity.class);
